...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.main_function_bind()  # 主界面功能按键绑定
    ...
    def yolo_init(self):
        ...
        self.select_model = self.model_box.currentText()              # 通过UI的模型选择框获取当前模型
        self.yolo_thread = QThread()                                  # YOLO检测单独做一个线程

        # 显示预测视频（左，右） yolo2main_trail_img是一个signal，在yolo.py中定义 通过信号连接到show_image函数
    ...
    # 模型加载  自动检索./models/ 下的模型文件 pt、engine
    def model_load(self):
        # 创建模型文件夹
    ...
    #区域标定
    def open_select_roi(self):
        video_path = self.yolo_predict.source
        select_roi(video_path)

    # 主页面各功能绑定
    def main_function_bind(self):
        self.src_file_button.clicked.connect(self.open_src_file) # 打开文件夹
        self.punctuation_detection_button.clicked.connect(self.open_select_roi)  # 区域标定
        ...
        self.ToggleBotton.clicked.connect(lambda: UIFuncitons.toggleMenu(self, True))  # 菜单栏
        self.settings_button.clicked.connect(lambda: UIFuncitons.settingBox(self, True)) # 设置

    # 模型参数绑定
    def model_bind(self):
        self.model_box.currentTextChanged.connect(self.change_model)    # 模型选择框
        self.iou_spinbox.valueChanged.connect(lambda x:self.change_val(x, 'iou_spinbox'))    # iou bar  交互比
        self.iou_slider.valueChanged.connect(lambda x:self.change_val(x, 'iou_slider'))      # iou scroll bar

        self.conf_spinbox.valueChanged.connect(lambda x:self.change_val(x, 'conf_spinbox'))  # conf box 置信度
        self.conf_slider.valueChanged.connect(lambda x:self.change_val(x, 'conf_slider'))    # conf scroll bar

        self.speed_spinbox.valueChanged.connect(lambda x:self.change_val(x, 'speed_spinbox'))# speed box 延时
        self.speed_slider.valueChanged.connect(lambda x:self.change_val(x, 'speed_slider'))  # speed scroll bar

        self.speed_sss.valueChanged.connect(lambda x: self.change_val(x, 'speed_sss'))  # speed_sss box 车流阈值
        self.speed_nnn.valueChanged.connect(lambda x:self.change_val(x, 'speed_nnn'))  # speed_nnn scroll bar
    ...
    def run_or_continue(self):
        ...
        if self.run_button.isChecked():
            ...
            if not self.yolo_thread.isRunning():
                self.yolo_thread.start()    # 启动检测线程
                self.main2yolo_begin_sgl.emit()   # 实时发送信号
        # 暂停
        else:
    ...
    def show_traffic_graph(self):
        ...
        if self.is_draw_thread:
            ...
            return
        self.draw_thread.start()    # 折线图线程开始
    ...
    def web_back_end(self):
        ...
        # 获取前端项目路径 (假设前端项目在同级目录的MTAS-guest文件夹中)
        frontend_dir = os.path.join(os.path.dirname(base_dir), 'MTAS-guest')
        ...
        if (self.web_flag):
            try:
                ...
                self.frontend_process = subprocess.Popen(
                    'npm run dev',
                    shell=True,
                    cwd=frontend_dir  # 设置工作目录为前端项目目录
                )
                
                MessageBox(self.close_button, title='提示', text='正在开启前后端...', time=2000, auto=True).exec()

                # 检查进程是否启动
                if self.server_process.pid is not None and self.frontend_process.pid is not None:
                    ...
                    DialogOver(parent=self, text="前后端已开启", title="开启成功", flags="success")
                else:
                    DialogOver(parent=self, text="进程启动失败", title="开启失败", flags="danger")
        ...
        # False是关闭前后端
            ...
            try:
                ...
                MessageBox(self.close_button, title='提示', text='正在关闭前后端...', time=2000, auto=True).exec()

                self.src_web_button.setText("开启网页端")
                self.web_flag = True
                DialogOver(parent=self, text="前后端已关闭", title="关闭成功", flags="success")
            except Exception as e:
    ...
    # 退出时退出线程，保存设置  把主要设置保存到config.json
