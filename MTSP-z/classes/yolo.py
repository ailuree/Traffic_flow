...
class YoloPredictor(BasePredictor, QObject):
    ...
    def load_roi_config(self):
        """加载ROI配置"""
        try:
            with open('./config/roi_config.txt', 'r') as f:
                self.roi_points = eval(f.read())
                print("已加载ROI配置")
        except:
            print("未找到ROI配置文件")
    ...
    def is_in_roi(self, box):
        ...
        if not self.roi_points:
            return True  # 如果没有设置ROI，则所有目标都有效
        ...
        # 检查点是否在多边形内
        # pointPolygonTest的参数顺序是：contour, point, measureDist
        result = cv2.pointPolygonTest(roi_poly, (float(center_x), float(center_y)), False)
        return result >= 0
    ...
    def res_address(self, img_res, result, height, width, model, out):
        ...
        # 绘制ROI区域
        if self.roi_points:
            roi_poly = np.array(self.roi_points, np.int32)
            overlay = img_box.copy()
            cv2.fillPoly(overlay, [roi_poly], (0, 255, 0, 128))
            alpha = 0.4
            img_box = cv2.addWeighted(overlay, alpha, img_box, 1-alpha, 0)
        ...
        # 如果有识别的
        elif result.boxes.id is not None:
            ...
            self.sum_of_count = len(detections)
